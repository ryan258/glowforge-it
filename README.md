...
| :----------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :--------------------------------------------------------------------- |
| `-w, --width`            | Target physical width in inches. The script scales the image to match this size at an exact 300 DPI before applying dithering. If `--height` isn't provided, it scales proportionally. Appends `--h{H}w{W}` to the output filename. | `None`                                                                 |
| `-h, --height`           | Target physical height in inches. The script scales the image to match this size at an exact 300 DPI before applying dithering. If `--width` isn't provided, it scales proportionally. Appends `--h{H}w{W}` to the output filename. | `None`                                                                 |
| `-i, --invert`           | Inverts the image immediately upon loading. Useful if you want the design to be engraved as a negative (light areas burn dark).                                                                                                     | `False`                                                                |
| `-c, --clean-solids`     | A flag that deeply scrubs AI-generated gradient artifacts. It aggressively snaps "almost black" to true black and "almost white" to true white right after loading the image.                                                       | `False` _(Note: The `gf` wrapper script automatically includes this!)_ |
| `-b, --black-threshold`  | `0-255` cutoff. Any pixels darker than this value are locked to pure solid black and completely bypassing the dithering algorithm. Use this to prevent white error-diffusion from "bleeding" into solid black text.                 | `0`                                                                    |
| `-W, --white-threshold`  | `0-255` cutoff. Any pixels lighter than this value are locked to pure solid white, keeping skies and backgrounds completely clean.                                                                                                  | `255`                                                                  |
| `-d, --dither-threshold` | The midpoint target point (`0-255`) where the Atkinson algorithm decides if a mid-tone pixel should round to black or white. Moving this up makes the overall image darker; moving it down makes it lighter.                        | `128`                                                                  |
| `--nb, --no-border`      | Disable the automatic 1px black border.                                                                                                                                                                                                   | `False`                                                                |
| `--help`                 | Show the command help output and exit.                                                                                                                                                                                                    | N/A                                                                    |
| `--input`                | Define a custom folder path or list of specific images to read directly from.                                                                                                                                                       | `input/`                                                               |
| `-o, --output`           | Define a custom folder path to save the processed files to.                                                                                                                                                                         | `output/`                                                              |
